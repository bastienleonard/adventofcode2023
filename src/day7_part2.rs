...
// TODO: decide between tuple- or struct-like attributes and be consistent
// TODO: same for naming ("regular"/"value")
#[derive(Copy, Clone, Hash, Eq, PartialEq, Debug)]
enum InitialCard {
    Value(CardValue),
    Joker
...
#[derive(Debug)]
struct Bid {
    // TODO: use array of other immutable type?
...
fn hand_type(cards: &[FixedUpCard]) -> HandType {
    ...
    for &card in cards {
        let key: u32 = match card {
            FixedUpCard::Regular { value } => value.value(), // TODO: value.value()
